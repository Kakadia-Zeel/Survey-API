...
exports.answerSurvey = async(req, res, next) => {
    ...
    for (var ans in req.body.data) {
        ...
        const yesanswer = `UPDATE question SET yes = yes+1 WHERE questionId = ?`;
        const yesanswerParams = [req.body.data[ans].questionId];
        const noanswer = `UPDATE question SET no = no+1 WHERE questionId = ?`;
        const noanswerParams = [req.body.data[ans].questionId];


        if (req.body.data[ans].ans == "true") {
            try {
                db.all(yesanswer, yesanswerParams, (err) => {
                    if (err) {
                        console.log(err);
                    }
                })
        ...
        if (req.body.data[ans].ans == "false") {
            try {
                db.all(noanswer, noanswerParams, (err) => {
                    if (err) {
                        console.log(err);
                    }
                })
